...
const Daftar = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { email, password, name, password_confirmation, role } = user;

    if (!email || !password || !name || !password_confirmation)
      return setMessage({
        ...message,
        text: "Email, nama, atau kata sandi tidak boleh kosong",
        msg: true,
        status: 400,
      });
    ...
    const data = await dispatch(registerUser({ email, password, name, password_confirmation, role }));
